...
class Preprocessing_rnn:
    ...
    def custom_scaling(self, X_train: pd.DataFrame, X_val: pd.DataFrame) -> tuple:
        ...
        # All other columns are kept as is
        cols_passthrough = [c for c in X_train.columns if c not in cols_pr + cols_rsds + cols_tas + col_lat + col_co2_nitro]
        ...
        if col_lat:
            ...
            val_lat = pd.DataFrame(np.cos(X_val[col_lat]/90), columns=col_lat, index=X_val.index)
        else:
            train_lat, val_lat = pd.DataFrame(), pd.DataFrame()
