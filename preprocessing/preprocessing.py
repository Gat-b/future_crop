...
class Preprocessing_ml:
    ...
    def __init__(self, raw_data_path='../raw_data', processed_data_path='../processed_data', crop='wheat', file='train'):
    ...
    def load_raw_data(self) -> pd.DataFrame:
        ...
        wheat_train_datasets = [
            {'file_name': 'pr_', 'path': self.raw_data_path / f'pr_{self.crop}_{self.file}parquet'},
            {'file_name': 'soil_co2_', 'path': self.raw_data_path / f'soil_co2_{self.crop}_{self.file}parquet'},
            {'file_name': 'tas_', 'path': self.raw_data_path / f'tas_{self.crop}_{self.file}parquet'},
            {'file_name': 'tasmin_', 'path': self.raw_data_path / f'tasmin_{self.crop}_{self.file}parquet'},
            {'file_name': 'tasmax_', 'path': self.raw_data_path / f'tasmax_{self.crop}_{self.file}parquet'},
            {'file_name': 'rsds_', 'path': self.raw_data_path / f'rsds_{self.crop}_{self.file}parquet'}]

        if self.file == 'train':
            wheat_train_datasets.append(
                {'file_name': '', 'path': self.raw_data_path / f'{self.file}_solutions_{self.crop}.parquet'})
    ...
    def feature_engineering(self, X: pd.DataFrame)-> pd.DataFrame:
        ...
        X = pd.concat([year, constant, texture, mean_pr,median_pr, sum_pr,min_pr,max_pr, rolling_30_days_pr,
                    ...
                    mean_rsds, median_rsds, sum_rsds,min_rsds,max_rsds, region_encoded, X_raw_features], axis=1)
        return X
    ...
    def custom_scaling(self, X_train: pd.DataFrame, X_val: pd.DataFrame) -> tuple:
        ...
        col_geo = ['lat', 'lon']
        col_co2_nitro = ['soil_co2_co2', 'soil_co2_nitrogen']
        
        # All other columns are kept as is
        cols_passthrough = [c for c in X_train.columns if c not in cols_pr + cols_rsds + cols_tas + col_geo + col_co2_nitro]
        ...
        # robust_scaling_pr
        if cols_pr:
            train_pr = pd.DataFrame(scaler_pr.fit_transform(X_train[cols_pr + col_co2_nitro]), 
                                    columns=cols_pr + col_co2_nitro, index=X_train.index)
            val_pr = pd.DataFrame(scaler_pr.transform(X_val[cols_pr + col_co2_nitro]), 
                                  columns=cols_pr + col_co2_nitro, index=X_val.index)
        else:
        ...
        if col_geo:
            train_lat = pd.DataFrame(np.cos(X_train[col_geo]/90), columns=col_geo, index=X_train.index)
            val_lat = pd.DataFrame(np.cos(X_val[col_geo]/90), columns=col_geo, index=X_val.index)
        else:
        ...
        ### DEBUGGING ###
        print(train_pr.shape, "\n", f"columns = {train_pr.columns}")
        print(val_pr.shape)
        print(train_tas.shape)
        print(val_tas.shape)
        print(train_lat.shape)
        print(val_lat.shape)
        print(train_pass.shape, "\n", f"columns = {train_pass.columns}")
        print(val_pass.shape)
...
if __name__ == "__main__":
    ...
    preproc = Preprocessing_ml(raw_data_path='raw_data', processed_data_path='processed_data', crop='wheat')
